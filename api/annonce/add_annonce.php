...
if (isset($id_user, $_POST['nom'], $_POST['description'], $_FILES['image'])) { //verifier si les champs sont renseigner
    $nom = $_POST['nom'];
    ...
    if ($prophile['error'] === UPLOAD_ERR_OK) {
        ...
        // Vérifiez si l'ann existe déjà pour obtenir son identifiant
        $stmt = $pdo->prepare("SELECT id FROM annonce WHERE id_user = :id_user AND nom = :nom");
        ...
        try {
            $sql = "INSERT INTO annonce (id_user, nom, description) VALUES (:id_user, :nom, :description)";
